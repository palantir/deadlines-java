...
import com.palantir.deadlines.DeadlineObserver.RequestDecodingAdapter;
import com.palantir.deadlines.DeadlineObserver.RequestEncodingAdapter;
...
class DeadlineObserverTest {

    @Test
    public void test_duration_to_header_value() {
        Duration duration = Duration.ofMillis(1523);
        String headerValue = DeadlineObserver.durationToHeaderValue(duration);
    ...
    public void test_header_value_to_duration() {
        ...
        Duration duration = DeadlineObserver.headerValueToDuration(headerValue);
    ...
    public void can_encode_to_request() {
        try (CloseableTracer tracer = CloseableTracer.startSpan("test")) {
            DeadlineObserver observer = new DeadlineObserver(new DefaultTaggedMetricRegistry());
            DummyRequest request = new DummyRequest();
            Duration deadline = Duration.ofSeconds(1);
            observer.encodeToRequest(deadline, request, new DummyRequestEncoder());

            assertThat(request.getFirstHeader(DeadlinesHttpHeaders.EXPECT_WITHIN))
                    .hasValueSatisfying(s -> {
                        String expected = DeadlineObserver.durationToHeaderValue(deadline);
    ...
    public void can_parse_from_request() {
        try (CloseableTracer tracer = CloseableTracer.startSpan("test")) {
            ...
            request.setHeader(
                    DeadlinesHttpHeaders.EXPECT_WITHIN, DeadlineObserver.durationToHeaderValue(providedDeadline));
            observer.parseFromRequest(request, new DummyRequestDecoder());

            Optional<Duration> remaining = observer.getRemainingDeadline();
    ...
    public void encode_to_request_uses_smaller_deadline_from_internal_state() {
        try (CloseableTracer tracer = CloseableTracer.startSpan("test")) {
            ...
            inboundRequest.setHeader(
                    DeadlinesHttpHeaders.EXPECT_WITHIN, DeadlineObserver.durationToHeaderValue(Duration.ofSeconds(1)));
            observer.parseFromRequest(inboundRequest, new DummyRequestDecoder());

            Optional<Duration> stateDeadline = observer.getRemainingDeadline();
            ...
            observer.encodeToRequest(providedDeadline, outboundRequest, new DummyRequestEncoder());

            assertThat(outboundRequest.getFirstHeader(DeadlinesHttpHeaders.EXPECT_WITHIN))
                    .hasValueSatisfying(h -> {
                        Duration parsed = DeadlineObserver.headerValueToDuration(h);
    ...
    public void encode_to_request_uses_smaller_deadline_from_argument() {
        try (CloseableTracer tracer = CloseableTracer.startSpan("test")) {
            ...
            DummyRequest inboundRequest = new DummyRequest();
            inboundRequest.setHeader(
                    DeadlinesHttpHeaders.EXPECT_WITHIN, DeadlineObserver.durationToHeaderValue(Duration.ofSeconds(2)));
            observer.parseFromRequest(inboundRequest, new DummyRequestDecoder());
    ...
    public void parse_from_request_noop_when_no_header_present() {
        try (CloseableTracer tracer = CloseableTracer.startSpan("test")) {
            ...
            DummyRequest request = new DummyRequest();
            observer.parseFromRequest(request, new DummyRequestDecoder());
            assertThat(observer.getRemainingDeadline()).isEmpty();
        }
    }
    ...
    public void parse_from_request_noop_when_no_trace() {
        ...
        DummyRequest request = new DummyRequest();
        Duration providedDeadline = Duration.ofSeconds(1);
        request.setHeader(DeadlinesHttpHeaders.EXPECT_WITHIN, DeadlineObserver.durationToHeaderValue(providedDeadline));
        observer.parseFromRequest(request, new DummyRequestDecoder());
        assertThat(observer.getRemainingDeadline()).isEmpty();
    }
